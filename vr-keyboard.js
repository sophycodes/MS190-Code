...
const ResponseStorage = {
  ...
  // 3D Grid layout settings - cards spread UP, OUT, and BACK
  gridConfig: {
    // Horizontal (X axis)
    columns: 5,
    colSpacing: 2.8,
    
    // Vertical (Y axis) - cards go UP
    rows: 4,              // Rows per depth layer
    rowSpacing: 2.0,      // Vertical space between rows
    startY: 2,            // Bottom row Y position
    
    // Depth (Z axis) - cards go BACK
    depthLayers: 5,       // How many depth layers
    depthSpacing: 3.5,    // Z space between layers
    startZ: -5,           // First layer Z position
  ...
  // Calculate 3D grid position for a card based on its index
  getGridPosition(questionId) {
    ...
    const baseY = config.startY + (row * config.rowSpacing);  // Goes UP
    ...
    // Rotation: slight tilt only, Y stays 0 to face forward
    const rotation = {
      x: (Math.random() - 0.5) * 6,
      y: 0,
      z: (Math.random() - 0.5) * 8
  ...
  async save(questionId, text) {
    // Get grid-based position (no overlaps!)
...
AFRAME.registerComponent('vr-keyboard', {
  ...
  async loadSavedResponses() {
    ...
    if (responses.length > 0) {
      ...
      // Initialize card count for this question
      ...
      if (container) {
        responses.forEach((response, index) => {
          ...
          setTimeout(() => {
            ...
            // But update the card count so new cards continue the grid
            ResponseStorage.cardCounts[questionId] = index + 1;
      ...
      // Initialize count to 0 for new questions
      ResponseStorage.cardCounts[questionId] = 0;
    }
  },
